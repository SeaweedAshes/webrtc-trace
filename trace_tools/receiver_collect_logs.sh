...
main() {
  ...
  if [[ "$client_status" -ne 0 && "$client_status" -ne 124 ]]; then
    if (( media_span_sec >= min_span_sec )); then
      log "WARNING: receiver client exited with status $client_status after usable ${media_span_sec}s log span; keeping bundle"
    else
      die "receiver client exited with status $client_status after only ${media_span_sec}s log span; check $client_log"
    fi
  fi
