...
int NetEqImpl::InsertPacket(const RTPHeader& rtp_header,
  ...
  {
    static FILE* audio_packet_log =
        rtc::OpenCsvLog("audio_packet_inserts.csv",
                        "wall_time_ms,seq_num,rtp_timestamp,payload_size\n");
    if (audio_packet_log) {
      fprintf(audio_packet_log, "%lld,%u,%u,%zu\n",
              (long long)env_.clock().CurrentTime().ms(),
              (unsigned)rtp_header.sequenceNumber,
              (unsigned)rtp_header.timestamp, payload.size());
    }
  }
