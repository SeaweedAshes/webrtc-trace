...
void PacingController::ProcessPackets() {
  ...
  if (data_sent > DataSize::Zero()) {
    static FILE* pacer_log = rtc::OpenCsvLog(
        "pacer_rate.csv",
        "timestamp_ms,batch_bytes,packets_sent,pacing_rate_bps,"
        "queue_packets,queue_bytes,is_probing\n");
    if (pacer_log) {
      fprintf(pacer_log, "%lld,%lld,%d,%lld,%d,%lld,%d\n",
              (long long)now.ms(),
              (long long)data_sent.bytes(),
              packets_sent,
              (long long)pacing_rate_.bps(),
              (int)packet_queue_.SizeInPackets(),
              (long long)packet_queue_.SizeInPayloadBytes().bytes(),
              (int)is_probing);
    }
  }
