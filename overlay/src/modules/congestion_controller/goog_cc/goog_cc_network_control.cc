...
void GoogCcNetworkController::MaybeTriggerOnNetworkChanged(
  ...
  if ((loss_based_target_rate != last_loss_based_target_rate_) ||
    ...
    DataRate pacing_rate =
        std::max(min_total_allocated_bitrate_, loss_based_target_rate) *
        pacing_factor_.value_or(first_transport_feedback_received_
                                    ? kDefaultPaceMultiplierWithSendSideBwe
                                    : kDefaultPaceMultiplier);
    DataRate padding_rate =
        (loss_based_state == LossBasedState::kIncreaseUsingPadding)
            ? std::max(max_padding_rate_, loss_based_target_rate)
            : max_padding_rate_;
    padding_rate = std::min(padding_rate, pushback_target_rate);
    {
      static FILE* bwe_log = rtc::OpenCsvLog(
          "bwe_target.csv",
          "timestamp_ms,target_bps,pushback_target_bps,"
          "fraction_loss_x256,rtt_ms,loss_based_state,"
          "pacing_rate_bps,padding_rate_bps\n");
      if (bwe_log) {
        fprintf(bwe_log, "%lld,%lld,%lld,%u,%lld,%d,%lld,%lld\n",
                (long long)at_time.ms(),
                (long long)loss_based_target_rate.bps(),
                (long long)pushback_target_rate.bps(),
                (unsigned)fraction_loss,
                (long long)round_trip_time.ms(),
                (int)loss_based_state,
                (long long)pacing_rate.bps(),
                (long long)padding_rate.bps());
      }
    }
  }
}
