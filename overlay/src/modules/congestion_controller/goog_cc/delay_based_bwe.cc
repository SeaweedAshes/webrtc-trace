...
DelayBasedBwe::Result DelayBasedBwe::MaybeUpdateEstimate(
  ...
  {
    static FILE* dbs_log = rtc::OpenCsvLog(
        "delay_bwe_state.csv",
        "at_time_ms,detector_state,target_bps,recovered_from_overuse\n");
    if (dbs_log) {
      fprintf(dbs_log, "%lld,%d,%lld,%d\n",
              (long long)at_time.ms(),
              (int)detector_state,
              (long long)(result.updated ? result.target_bitrate.bps()
                                         : prev_bitrate_.bps()),
