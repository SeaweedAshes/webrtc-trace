...
int FrameBuffer::GetNumberOfBufferedContinuousTemporalUnits() const {
  if (!last_continuous_temporal_unit_frame_id_) {
    return 0;
  }

  int num_buffered_continuous_temporal_units = 0;
  for (auto it = frames_.begin(); it != frames_.end(); ++it) {
    if (GetFrameId(it) > *last_continuous_temporal_unit_frame_id_) {
      break;
    }
    if (it->second.continuous && IsLastFrameInTemporalUnit(it)) {
      ++num_buffered_continuous_temporal_units;
    }
  }
  return num_buffered_continuous_temporal_units;
}
