...
TEST(FrameBuffer3Test, NumberOfBufferedContinuousTemporalUnits) {
  FieldTrials field_trials = CreateTestFieldTrials();
  FrameBuffer buffer(/*max_frame_slots=*/10, /*max_decode_history=*/100,
                     field_trials);
  EXPECT_THAT(buffer.GetNumberOfBufferedContinuousTemporalUnits(), Eq(0));

  EXPECT_TRUE(buffer.InsertFrame(
      test::FakeFrameBuilder().Time(10).Id(1).AsLast().Build()));
  EXPECT_TRUE(buffer.InsertFrame(
      test::FakeFrameBuilder().Time(20).Id(2).Refs({1}).AsLast().Build()));
  EXPECT_THAT(buffer.GetNumberOfBufferedContinuousTemporalUnits(), Eq(2));

  EXPECT_THAT(buffer.ExtractNextDecodableTemporalUnit(),
              ElementsAre(FrameWithId(1)));
  EXPECT_THAT(buffer.GetNumberOfBufferedContinuousTemporalUnits(), Eq(1));
