...
FILE* ReceiverDecodedFramesLog() {
  static FILE* f = rtc::OpenCsvLog(
      "receiver_decoded_frames.csv",
      "timestamp_ms,rtp_timestamp,ntp_time_ms,render_time_ms,width,height,"
      "frame_size_bytes,packet_count\n");
  return f;
}

FILE* ReceiverRenderedFramesLog() {
  static FILE* f = rtc::OpenCsvLog(
      "receiver_rendered_frames.csv",
      "timestamp_ms,rtp_timestamp,ntp_time_ms,render_time_ms,width,height,"
      "frame_size_bytes,packet_count\n");
  return f;
}
...
void VideoReceiveStream2::OnFrame(const VideoFrame& video_frame) {
  const int64_t decoded_time_ms = env_.clock().CurrentTime().ms();
  RtpPacketInfos packet_infos = video_frame.packet_infos();
  if (FILE* decoded_log = ReceiverDecodedFramesLog()) {
    fprintf(decoded_log, "%lld,%u,%lld,%lld,%d,%d,%u,%zu\n",
            static_cast<long long>(decoded_time_ms),
            static_cast<unsigned>(video_frame.rtp_timestamp()),
            static_cast<long long>(video_frame.ntp_time_ms()),
            static_cast<long long>(video_frame.render_time_ms()),
            video_frame.width(), video_frame.height(),
            static_cast<unsigned>(video_frame.size()), packet_infos.size());
  }
  ...
  // for `packet_infos`.
  const int64_t rendered_time_ms = env_.clock().CurrentTime().ms();
  if (FILE* rendered_log = ReceiverRenderedFramesLog()) {
    fprintf(rendered_log, "%lld,%u,%lld,%lld,%d,%d,%u,%zu\n",
            static_cast<long long>(rendered_time_ms),
            static_cast<unsigned>(video_frame.rtp_timestamp()),
            static_cast<long long>(video_frame.ntp_time_ms()),
            static_cast<long long>(video_frame.render_time_ms()),
            video_frame.width(), video_frame.height(),
