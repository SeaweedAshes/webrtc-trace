  ...
  if (num_frames_rendered_ > 0) {
    ...
    if (!is_paused_) {
      ...
      if (was_freeze) {
        ...
        {
          static FILE* freeze_log = rtc::OpenCsvLog(
              "video_freeze.csv",
              "timestamp_ms,freeze_duration_ms,total_freeze_count\n");
          if (freeze_log) {
            fprintf(freeze_log, "%lld,%lld,%lld\n",
                    (long long)frame_meta.decode_timestamp.ms(),
                    (long long)interframe_delay_ms,
                    (long long)freezes_durations_.NumSamples());
          }
        }
      } else {
