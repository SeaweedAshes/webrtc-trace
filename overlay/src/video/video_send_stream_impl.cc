...
FILE* SenderEncodedFramesLog() {
  static FILE* f = rtc::OpenCsvLog(
      "sender_encoded_frames.csv",
      "timestamp_ms,rtp_timestamp,ntp_time_ms,capture_time_ms,width,height,"
      "encoded_size_bytes,frame_type,spatial_idx,temporal_idx,qp,"
      "encoder_target_bps\n");
  return f;
}

FILE* SenderFrameDropsLog() {
  static FILE* f = rtc::OpenCsvLog(
      "sender_frame_drops.csv",
      "timestamp_ms,rtp_timestamp,spatial_idx,is_end_of_temporal_unit,"
      "encoder_target_bps,drop_reason\n");
  return f;
}
...
EncodedImageCallback::Result VideoSendStreamImpl::OnEncodedImage(
  ...
  if (FILE* encoded_log = SenderEncodedFramesLog()) {
    fprintf(encoded_log,
            "%lld,%u,%lld,%lld,%u,%u,%zu,%d,%d,%d,%d,%lld\n",
            static_cast<long long>(env_.clock().TimeInMilliseconds()),
            static_cast<unsigned>(encoded_image.RtpTimestamp()),
            static_cast<long long>(encoded_image.NtpTimeMs()),
            static_cast<long long>(encoded_image.capture_time_ms_),
            static_cast<unsigned>(encoded_image._encodedWidth),
            static_cast<unsigned>(encoded_image._encodedHeight),
            encoded_image.size(),
            static_cast<int>(encoded_image._frameType),
            encoded_image.SpatialIndex().value_or(-1),
            encoded_image.TemporalIndex().value_or(-1), encoded_image.qp_,
            static_cast<long long>(
                encoder_target_bps_for_logging_.load(std::memory_order_relaxed)));
  }
...
void VideoSendStreamImpl::OnFrameDropped(uint32_t rtp_timestamp,
  ...
  activity_ = true;
  if (FILE* drop_log = SenderFrameDropsLog()) {
    fprintf(drop_log, "%lld,%u,%d,%d,%lld,%s\n",
            static_cast<long long>(env_.clock().TimeInMilliseconds()),
            static_cast<unsigned>(rtp_timestamp), spatial_id,
            static_cast<int>(is_end_of_temporal_unit),
            static_cast<long long>(
                encoder_target_bps_for_logging_.load(std::memory_order_relaxed)),
            "encoder_drop");
  }
...
uint32_t VideoSendStreamImpl::OnBitrateUpdated(BitrateAllocationUpdate update) {
  ...
  encoder_target_bps_for_logging_.store(encoder_target_rate_.bps(),
